...
// 加载当前颜色 (Firefox compatible)
async function loadCurrentColor() {
    try {
        const browser = window.chrome || window.browser;
        const tab = await getCurrentTab();
        const result = await browser.storage.local.get([`color_${tab.id}`]);
...
async function setColor(colorGenerator, actionName = 'setting color') {
    try {
        ...
        // 保存颜色到存储
        await browser.storage.local.set({ [`color_${tab.id}`]: color });
...
async function removeColor() {
    try {
        ...
        // 从存储中移除
        await browser.storage.local.remove([`color_${tab.id}`]);
        ...
        try {
            ...
            try {
                ...
                console.log('Script injection failed, the page may need to be refreshed');
            }
        }
...
// 注入content script（如果需要） (Firefox compatible)
async function injectContentScriptIfNeeded(tabId) {
    try {
        ...
        // Firefox使用传统API注入content script
        try {
            ...
            console.log('Content script injection failed, the page may need to be refreshed');
        }
    }
}

// 直接注入边框代码 (Firefox compatible)
async function injectBorderDirectly(tabId, color) {
    const browser = window.chrome || window.browser;
    
    // Firefox使用传统的tabs.executeScript API
    try {
        await browser.tabs.executeScript(tabId, {
            code: `
                (function(color) {
        ...
        console.log('Direct border injection failed, the page may need to be refreshed');
    }
}