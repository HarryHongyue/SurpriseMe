...
// Predefined colors array for the extension
...
async function loadCurrentColor() {
    try {
        ...
        const result = await browserAPI.storage.local.get([`color_${tab.id}`]);
...
async function setColor(colorGenerator, actionName = 'setting color') {
    try {
        ...
        // 保存颜色到存储
        await browserAPI.storage.local.set({ [`color_${tab.id}`]: color });
...
async function removeColor() {
    try {
        ...
        // 从存储中移除
        await browserAPI.storage.local.remove([`color_${tab.id}`]);
...
// Check if the page is restricted (browser pages, extensions, etc.)
    ...
    try {
        await browserAPI.scripting.executeScript({
            ...
            func: (color) => {
                ...
                if (!color) return; // If no color, just remove the border
        ...
        console.error('Error injecting border directly:', error);
